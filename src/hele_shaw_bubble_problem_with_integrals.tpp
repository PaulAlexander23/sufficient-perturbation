...
BubbleInChannelProblem<ELEMENT>::BubbleInChannelProblem() {
  ...
  CoM_Y_constraint_el_pt = new SelfReferentialVolumeConstraintElement(
      Problem_Parameter::global_CoM_Y_pt, CoM_Y_data_pt,
      index_of_traded_pressure);

  Integral_measures_el_pt = new SelfReferentialVolumeConstraintElement(
      Problem_Parameter::global_CoM_Y_pt, Integral_measures_data_pt,
      index_of_traded_pressure);
...
void BubbleInChannelProblem<ELEMENT>::doc_solution(const std::string &comment) {
  ...
  // Output boundaries
  sprintf(filename, "%s/boundaries_%i.dat",
          Problem_Parameter::Doc_info.directory().c_str(),
          Problem_Parameter::Doc_info.number());
  some_file.open(filename);
  ...
  for (unsigned i_measure = 0;
       i_measure < Problem_Parameter::n_integral_measures; i_measure++) {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::jack_solve(double Q, bool jack_arc,
                                                 unsigned n_iter, double sign) {
  if (jack_arc == true) {
    std::cout << " " << std::endl;
    std::cout << " " << std::endl;
    std::cout << "===========================================" << std::endl;
...
///-------------------------------------------------------------------------------
template <class ELEMENT> Problem *BubbleInChannelProblem<ELEMENT>::make_copy() {
...
template <class ELEMENT>
double BubbleInChannelProblem<ELEMENT>::global_temporal_error_norm() {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::reset_lagrangian_coordinates() {
  Fluid_mesh_pt->set_lagrangian_nodal_coordinates();
}

template <class ELEMENT>
double BubbleInChannelProblem<ELEMENT>::get_bubble_pressure() {
  return Bubble_pressure_data_pt->value(0);
}

template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::create_inflow_elements() {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::delete_inflow_elements() {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::delete_outflow_elements() {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::pin_all_nodal_positions() {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::unpin_all_nodal_positions() {
  unsigned n_node = Fluid_mesh_pt->nnode();
  for (unsigned inod = 0; inod < n_node; inod++) {
    Node *nod_pt = Fluid_mesh_pt->node_pt(inod);
    SolidNode *solid_node_pt = dynamic_cast<SolidNode *>(nod_pt);
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::pin_bubble_pressure() {
  for (unsigned ibound = First_bubble_boundary_id;
       ibound < First_bubble_boundary_id + 2; ibound++) {
    unsigned num_nod = Fluid_mesh_pt->nboundary_node(ibound);
    for (unsigned inod = 0; inod < num_nod; inod++) {
      // Get node
      Node *nod_pt = Fluid_mesh_pt->boundary_node_pt(ibound, inod);
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::pin_all_pressure() {
  unsigned n_node = Fluid_mesh_pt->nnode();
  for (unsigned inod = 0; inod < n_node; inod++) {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::unpin_all_pressure() {
  unsigned n_node = Fluid_mesh_pt->nnode();
  for (unsigned inod = 0; inod < n_node; inod++) {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::pin_curvature_equations() {
  //        unsigned nbound=Fluid_mesh_pt->nboundary();
  for (unsigned ibound = First_bubble_boundary_id;
       ibound < First_bubble_boundary_id + 2; ibound++) {
    unsigned num_nod = Fluid_mesh_pt->nboundary_node(ibound);
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::pin_tangential_lagrange() {
  //        unsigned nbound=Fluid_mesh_pt->nboundary();
  for (unsigned ibound = First_bubble_boundary_id;
       ibound < First_bubble_boundary_id + 2; ibound++) {
    unsigned num_nod = Fluid_mesh_pt->nboundary_node(ibound);
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::unpin_curvature_equations() {
  //        unsigned nbound=Fluid_mesh_pt->nboundary();
  for (unsigned ibound = First_bubble_boundary_id;
       ibound < First_bubble_boundary_id + 2; ibound++) {
    unsigned num_nod = Fluid_mesh_pt->nboundary_node(ibound);
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::reset_interface_equations() {
  //        unsigned nbound=Fluid_mesh_pt->nboundary();
  for (unsigned ibound = First_bubble_boundary_id;
       ibound < First_bubble_boundary_id + 2; ibound++) {
    unsigned num_nod = Fluid_mesh_pt->nboundary_node(ibound);
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::redo_equation_numbering() {
  cout << "Number of equations: " << this->assign_eqn_numbers() << std::endl;
}
...
/// Volume
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_V(double new_V) {
  Problem_Parameter::Volume = -new_V;
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_V() {
  return -Problem_Parameter::Volume;
}
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::increment_V(double dV) {
  set_V(get_V() + dV);
}

/// Frame speed (if pinned, frame does not move with bubble)
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_U(double new_U) {
  U_data_pt->set_value(0, new_U);
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_U() {
  return U_data_pt->value(0);
}
template <class ELEMENT> void BubbleInChannelProblem<ELEMENT>::pin_U() {
  U_data_pt->pin(0);
}
template <class ELEMENT> void BubbleInChannelProblem<ELEMENT>::unpin_U() {
  U_data_pt->unpin(0);
}

/// Bubble pressure  (volume constraint is not enforced if P is pinned)
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_P(double new_P) {
  Bubble_pressure_data_pt->set_value(0, new_P);
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_P() {
  return Bubble_pressure_data_pt->value(0);
}
template <class ELEMENT> void BubbleInChannelProblem<ELEMENT>::pin_P() {
  Bubble_pressure_data_pt->pin(0);
}
template <class ELEMENT> void BubbleInChannelProblem<ELEMENT>::unpin_P() {
  Bubble_pressure_data_pt->unpin(0);
}

/// Imposed flow rate
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_Q(double new_Q) {
  Q_inv_data_pt->set_value(0, 1.0 / new_Q);
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_Q() {
  return 1.0 / Q_inv_data_pt->value(0);
}
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::increment_Q(double d_Q) {
  double new_Q = get_Q() + d_Q;
  set_Q(new_Q);
}

template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_Q_inv() {
  return Q_inv_data_pt->value(0);
}

/// We measure CoM_Y through an integral constraint.
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_CoM_Y() {
  return CoM_Y_data_pt->value(0);
}
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_CoM_Y(double new_CoM_Y) {
  CoM_Y_data_pt->set_value(0, new_CoM_Y);
}

/// Channel geometry parameters
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_asymmetry(double new_asymmetry) {
  Asymmetry_data_pt->set_value(0, new_asymmetry);
}
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_h(double new_h) {
  *Problem_Parameter::global_Obstacle_height_pt = new_h;
}
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_w(double new_w) {
  *Problem_Parameter::global_Obstacle_width_pt = new_w;
}
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_alpha(double new_alpha) {
  *Problem_Parameter::global_alpha_pt = new_alpha;
}
template <class ELEMENT>
double BubbleInChannelProblem<ELEMENT>::get_asymmetry() {
  return Asymmetry_data_pt->value(0);
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_h() {
  return *Problem_Parameter::global_Obstacle_height_pt;
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_w() {
  return *Problem_Parameter::global_Obstacle_width_pt;
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_maj_rad() {
  return Problem_Parameter::Major_Radius;
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_min_rad() {
  return Problem_Parameter::Minor_Radius;
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_xcenter() {
  return Problem_Parameter::xcenter;
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_ycenter() {
  return Problem_Parameter::ycenter;
}
template <class ELEMENT>
unsigned BubbleInChannelProblem<ELEMENT>::get_circpts() {
  return Problem_Parameter::circpts;
}
template <class ELEMENT> double BubbleInChannelProblem<ELEMENT>::get_alpha() {
  return *Problem_Parameter::global_alpha_pt;
}
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::increment_h(double dh) {
  set_h(get_h() + dh);
}

template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::output_jacobian() {
...
template <class ELEMENT> void BubbleInChannelProblem<ELEMENT>::set_steady() {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::delete_CoM_X_constraint_elements() {
  // How many surface elements are in the surface mesh
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::delete_CoM_Y_constraint_elements() {
  // How many surface elements are in the surface mesh
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_steady_constraints() {
...
template <class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_steady_constraints_free_Q() {
  std::cout << "========================" << std::endl;
...
template <class ELEMENT> void BubbleInChannelProblem<ELEMENT>::pin_G() {
  G_data_pt->pin(0);
}
template <class ELEMENT> void BubbleInChannelProblem<ELEMENT>::pin_Q() {
  Q_inv_data_pt->pin(0);
}

template <class ELEMENT> void BubbleInChannelProblem<ELEMENT>::unpin_G() {
  G_data_pt->unpin(0);
}
template <class ELEMENT> void BubbleInChannelProblem<ELEMENT>::unpin_Q() {
  Q_inv_data_pt->unpin(0);
}

template <class ELEMENT>
bool BubbleInChannelProblem<ELEMENT>::get_jacobian_sign_change() {
