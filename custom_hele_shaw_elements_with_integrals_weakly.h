...
{
    /// This function should have obstacle width and height, asymmetry and possibly sharpness.
...
//     b = 1 - ( height * 0.5 *(tanh_plus - tanh_minus) * (1+rough) + waveCell );
//     b = 1 - ( height * 0.5 *(tanh_plus - tanh_minus) + waveCell );
    ...
    {
         OccluHeight_file << y << "   " << b << "   " << sdr << "   " << rd << "   " << c << std::endl;
    }
}
...
ofstream UpperWall_file;
 ...
 void upper_wall_flux_fct(const Vector<double>  &x,double &b, double &dbdt, Vector<double> &dbdx,Vector<double> &d_dbdt_dx)
 {
   channel_height_function(x,b);
   if(ignore_bulk_effects) {b=1; UpperWall_file << b << std::endl; }
   dbdt = 0.0;
...
{
    /// For slight simplicity, we can put the transverse curvature term as a spatially varying
    /// bubble pressure.
...
void bubble_pressure_gradient_function (const Vector<double>&x, Vector<double>& grad_pb)
{
    /// ALICE_FLAG
...
template<>
class FaceGeometry<MyNewElement>
