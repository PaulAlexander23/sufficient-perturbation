...
/// Actions after adapt: Rebuild the mesh of free surface elements
    ...
    double global_temporal_error_norm()
    {
        ...
        oomph_info << "Global temporal error norm: " << global_error << " over " << count << " interface nodes" << std::endl;
...
/// Update the problem specs before solve
	...
	{
	  std::cout<<" "<<std::endl;
	  std::cout<<"The solid jacobian in the interface elements will be filled in by fd"<<std::endl;
	}
      if(answer==false)
	{
	  std::cout<<" "<<std::endl;
	  std::cout<<"The solid jacobian in the interface elements will be filled in exactly"<<std::endl;
	}
      Problem_Parameter::fill_in_solid_jacobian_interface_by_fd = answer;
    }

    
    void perturb_to_second_order(Vector<complex<double> > &g,Vector<complex<double> > &varphi_0,Vector<complex<double> > &varphi_1,Vector<complex<double> > &varphi_2,double eps,double r)
    {
      int N = this->ndof();

      for(int i=0;i<N;i++)
	{
	  this->dof(i) += eps*2.0*real(g[i])*r + eps*eps*(2.0*real(varphi_0[i])*r*r + real(varphi_1[i])*r*r + real(varphi_2[i]));
	}
    }
			     ...
			     Vector<complex<double> > &varphi_3)
{
  ...
  std::cout<<"varphi_0_real_pressure is "<<varphi_0_real_pressure<<std::endl;
  std::cout<<"varphi_0_imag_pressure is "<<varphi_0_imag_pressure<<std::endl;
  ...
  std::cout<<"varphi_3_speed is "<<varphi_3_speed<<std::endl;
    ...
    void normalise_eigenvectors(Vector<complex<double> > &g, Vector<complex<double> > &g_adjoint)
    {
      std::cout<<" "<<std::endl;
    ...
    void solve_landau_equation(double tend,Vector<double> &solution,double dt,complex<double> &lambda,complex<double> &mu,double epsilon);

    void solve_landau_equation(Vector<double> &solution,double dt,double t,complex<double> &lambda,complex<double> &mu,double epsilon);
    ...
    complex<double> get_inner_product(Vector<complex<double> > &f,Vector<complex<double> > &g)
    {
    ...
    void get_inner_product(Vector<complex<double> > &f,Vector<complex<double> > &g,complex<double> &ans);
   
    void get_landau_residual(Vector<double> &u,Vector<double> &f,complex<double> &lambda,complex<double> &mu,double epsilon);
    
    void jack_solve(double Q_unsteady,bool jack_arc,unsigned n_iter,double sign);
    ...
    int create_bifurcation_diagram(double delta,complex<double> lambda,complex<double> mu,bool hopf1);
				       ...
				       int mode)
    {
      ...
      sprintf(filename,"RESLT/eigenvectors_%i.dat",
	      mode);
      ...
      for(int i=0;i<ndof;i++)
	{
      ...
      sprintf(filename,"RESLT/eigenvalues_0.dat");
      indata.open(filename);
      indata.precision(20);
				       ...
				       int mode)
    {
      ...
      sprintf(filename,"RESLT/eigenvectors_adjoint_%i.dat",
	      mode);
      indata.open(filename);
      indata.precision(20);
      ...
      for(int i=0;i<ndof;i++)
	{
      ...
      sprintf(filename,"RESLT/eigenvalues_adjoint_0.dat");
      indata.open(filename);
      indata.precision(20);
    ...
    Vector<complex<double> > fill_in_landau_coefficients(Vector<complex<double> > &g,
							 Vector<complex<double> > &g_adjoint,
							 Vector<complex<double> > &varphi_0,
							 Vector<complex<double> > &varphi_1,
							 Vector<complex<double> > &varphi_2,
    ...
    void get_hessian(DoubleVector &g1,DoubleVector &g2,vector<double> &u0,DoubleVector &f);

    void get_tressian(Vector<complex<double> > &g1,Vector<complex<double> > &g2,vector<double> &u0,Vector<complex<double> > &f);
    
    void get_tressian_new(Vector<complex<double> > &g1,Vector<complex<double> > &g2,Vector<complex<double> > &g3,vector<double> &u0,Vector<complex<double> > &f);

    void get_tressian_new1(DoubleVector &g1,DoubleVector &g2,DoubleVector &g3,Vector<double> &u0,DoubleVector &f);
    
    void get_mass(DoubleVector &g1,DoubleVector &g2,vector<double> &u0,DoubleVector &f,bool fd=false);
    ...
    void create_complex_mass_matrix(CRComplexMatrix &mass_complex,complex<double> mult);

    void create_complex_jacobian_matrix(CRComplexMatrix &mass_complex,complex<double> mult);
	...
	{
	  if(isinf(real(eigenvalues[n]))!=true && isnan(real(eigenvalues[n]))!=true)
	    {
	      if(silent==false)
		{
	  ...
	  if(doc_eigenvector==true)
	    {
	      if(silent==false)
		{
		  std::cout<<"Doc eigenvector with eigenvalue" << real(eigenvalues[n]) << " " << imag(eigenvalues[n]) << " " << std::endl;
		}
      ...
      for(int i=1;i<N_theta;i++)
	{
      ...
      ofstream outdata;
      char filename[100];
      sprintf(filename,"RESLT/eigenmode_data.dat");
      outdata.open(filename);
      for(int m=0;m<N_theta;m++)
	{
	  for(unsigned i=0;i<n_dof;i++)
	    {
	      dof(i) = backup_1[i] + epsilon*(cos(theta[m])*eigenvectors[eigenmode][i] - sin(theta[m])*eigenvectors[conjugate][i]);
	    }
	  actions_after_change_in_bifurcation_parameter();
	  outdata << theta[m]<<" "<<get_CoM_Y()<<" "<<get_y_max()<<" "<<get_y_min()<<std::endl;
	  epsilon = epsilon + 1;
	}
      outdata.close();
    }
      ...
      for(int i=0;i<N;i++)
	{
      ...
      std::cout<<"Inner product eigenvalue is "<<evalue2/evalue1<<std::endl;
      ...
      for(int i=0;i<N;i++)
	{
      ...
      for(int i=0;i<N;i++)
	{
	  result[i] = A[i] + 1i*B[i] - (evalue_real + 1i*evalue_imag)*(C[i] + 1i*D[i]);
	}
      ...
      for(int i=0;i<N;i++)
	{
      ...
      std::cout<<" "<<std::endl;
      std::cout<<"The max of M*s*g - J*g is "<<result[index]<<" at "<<index<<std::endl;

      std::cout<<" "<<std::endl;
      
      ofstream outdata;
      char filename[100];
      sprintf(filename,"RESLT/result_%i.dat",m);
      ...
      for(unsigned m=0;m<n_eval;m++)
	{
	  ...
	  std::cout<<real(eigenvalues[m])<<" + i"<<imag(eigenvalues[m])<<std::endl;
	  std::cout<<"====================="<<std::endl;
	  std::cout<<"====================="<<std::endl;
      ...
      for(unsigned m=0;m<1000;m++)
	{
      ...
      std::cout<<"====================="<<std::endl;
      std::cout<<"====================="<<std::endl;
      std::cout<<real(eigenvalues[eigenmode])<<" + i"<<imag(eigenvalues[eigenmode])<<std::endl;
      std::cout<<"====================="<<std::endl;
      std::cout<<"====================="<<std::endl;
      ...
      some_file <<eigenmode<<" "<< epsilon<<" "<< " "<<theta<<endl;
      some_file.close();

      for(unsigned i =0;i<n_dof;i++)
	{
	  dof(i) = 100*(cos(theta)*eigenvectors[eigenmode][i] - sin(theta)*eigenvectors[conjugate][i]);
	}
      ...
      for(unsigned i=0;i<n_dof;i++)
	{
	  dof(i) = backup_1[i] + epsilon*(cos(theta)*eigenvectors[eigenmode][i] - sin(theta)*eigenvectors[conjugate][i]);
	}
    ...
    void solve_for_eigenproblem_fold(Vector<complex<double> > &eigenvalues,Vector<DoubleVector> &eigenvectors)
    {
      ...
      for(unsigned n=0;n<10;n++)
	{
      ...
      std::cout<<"There are "<< sensible_eigenvalues << " reasonable eigenvalues" << std::endl;
	...
	{	  
	  ...
	  solve_for_eigenproblem_fold(eigenvalues,eigenvectors);
	  eval_index = sort_complex_eigenvalues(&real_evalue_temp,&imag_evalue_temp);
	  reset_lagrangian_coordinates();
      ...
      for(unsigned n=0;n<n_dof;n++)
	{
      ...
      //Doc the eigenvectors
	      ...
	      std::cout<<"Doc eigenvector with eigenvalue" << real(eigenvalues[eval_index]) << " " << imag(eigenvalues[eval_index]) << " " << std::endl;
	      ...
	      for(unsigned i=0;i<n_dof;i++)
		{
	      ...
	      std::cout<<"Eigenvalue is " << real_evalue_temp<< " + i"<<imag_evalue_temp<<std::endl;
	      std::cout<<"================"<<std::endl;
	      Q_inv_data_pt->pin(0);
	      U_data_pt->unpin(0);
	      assign_eqn_numbers();
    ...
    unsigned sort_real_eigenvalues(double *real_evalue_min,double *imag_evalue_min)
    {
      ...
      for(int m=0;m<10;m++)
	{
      ...
      std::cout<<" "<<std::endl;
      std::cout<<"There are "<<real_index<<" eigenvalues which are real"<<std::endl;
      ...
      for(int m=0;m<real_index;m++)
	{
      ...
      double real_evalue_temp = Problem_Parameter::vector_of_eigenvalues_rp[real[0]];
      unsigned evalue_index = real[0];
      
      for(int m=1;m<real_index;m++)
	{

	  if(Problem_Parameter::vector_of_eigenvalues_rp[real[m]] < real_evalue_temp)
	    {
      ...
      *imag_evalue_min = Problem_Parameter::vector_of_eigenvalues_ip[evalue_index];
      *real_evalue_min = Problem_Parameter::vector_of_eigenvalues_rp[evalue_index];
      std::cout<<"==========================="<<std::endl;
    ...
    unsigned sort_complex_eigenvalues(double *real_evalue_min,double *imag_evalue_min)
    {
      ...
      for(int m=0;m<10;m++)
	{
      ...
      std::cout<<" "<<std::endl;
      std::cout<<"There are "<<complex_index<<" eigenvalues with an imaginary part"<<std::endl;
      std::cout<<"They are at.."<<std::endl;
      for(int m=0;m<complex_index;m++)
	{
      ...
      double complex_evalue_temp = Problem_Parameter::vector_of_eigenvalues_rp[imag[0]];
      unsigned evalue_index = imag[0];
      
      for(int m=1;m<complex_index;m++)
	{

	  if(Problem_Parameter::vector_of_eigenvalues_rp[imag[m]] < complex_evalue_temp)
	    {
	      complex_evalue_temp = Problem_Parameter::vector_of_eigenvalues_rp[imag[m]];
	      evalue_index = imag[m];
	    }
	  else
	    {
    ...
    void output_eigenvector_boundaries(unsigned m,bool pert=false)
    {
      ...
      if(pert==false)
	{
	  sprintf(filename,"%s/boundaries_evectors_%i_%i.dat",
      ...
      if(pert==true)
	{
	  sprintf(filename,"%s/boundaries_pert_evectors_%i_%i.dat",
		  Problem_Parameter::Doc_info.directory().c_str(),
		  Problem_Parameter::Doc_info.number(),
		  m);
	}
 ...
 void output_eigenvectors_and_eigenvalues(Vector<complex<double> > &eigenvalues,Vector<DoubleVector> &eigenvectors)
 {
   ...
   sprintf(filename,"RESLT/eigenvalues_%i.dat",
	   Problem_Parameter::Doc_info.number());
   some_file.open(filename);
   some_file.precision(20);
 ...
 void output_adjoint_eigenvectors_and_eigenvalues(Vector<complex<double> > &eigenvalues,Vector<DoubleVector> &eigenvectors)
 {
   double n_evals = eigenvalues.size();
   double n_dof = ndof();
   ofstream some_file;
   ...
   sprintf(filename,"RESLT/eigenvalues_adjoint_%i.dat",
	   Problem_Parameter::Doc_info.number());
   some_file.open(filename);
   some_file.precision(20);
 ...
 void output_eigenmatrices()
 {
   ...
   sprintf(filename,"RESLT/mass_matrix_%i.dat",
	   Problem_Parameter::Doc_info.number());
   ...
   sprintf(filename,"RESLT/jacobian_matrix_%i.dat",
	   Problem_Parameter::Doc_info.number());
...
/// Compute the error estimates and assign to elements for plotting
    ...
    void create_inflow_elements()
    {
        ...
        unsigned n_inflow_element = Fluid_mesh_pt->nboundary_element (boundary_for_flux_elements);
        for (unsigned e = 0; e < n_inflow_element; e++)
        {
            ELEMENT* bulk_elem_pt = dynamic_cast<ELEMENT*> (Fluid_mesh_pt->boundary_element_pt (boundary_for_flux_elements, e) );
            //Find the index of the face of element e along boundary b
            int face_index = Fluid_mesh_pt->face_index_at_boundary (boundary_for_flux_elements, e);
            HeleShawFluxElement<ELEMENT>* el_pt = new HeleShawFluxElement<ELEMENT> (bulk_elem_pt, face_index);
            Inflow_mesh_pt->add_element_pt (el_pt);
            el_pt->flux_fct_pt() = &Problem_Parameter::normal_flux_behind_bubble;//JACK - THIS SAYS THAT THE flux_fc_pt() = normal_flux_behind_bubble

            /// This one is important!
            el_pt->add_external_data (G_data_pt,true);//JACK - THIS TELLS THE FLUX ELEMENT ABOUT THE PRESCRIBED G
        }
    ...
    {
        for(unsigned ibound=First_bubble_boundary_id; ibound<First_bubble_boundary_id+2; ibound++)
        {
...
//        set_tip_node_pt();
    ...
    void set_steady_constraints_free_Q()
    {
        ...
        std::cout << "G is pinned. Q varies to meet position constraint"  << std::endl;
        std::cout << "========================" << std::endl;
...
// previous timsteps.
    ...
    Problem_Parameter::global_Obstacle_height_pt = Obstacle_height_data_pt->value_pt(0);
    ...
    Problem_Parameter::global_Obstacle_width_pt = Obstacle_width_data_pt->value_pt(0);
    ...
    Problem_Parameter::global_Bubble_pressure_pt = Bubble_pressure_data_pt->value_pt (0);
    Bubble_pressure_data_pt->unpin(0);

    /// Or might vary Q.

    U_data_pt = new Data(1);//Jack - Needs to point somewhere
    this->add_global_data (U_data_pt);//The generic problem class adds U_data_pt to list of unknowns
    U_data_pt->set_value(0,initial_Frame_speed);//Sets value (and probably where it points)
    Problem_Parameter::global_Frame_speed_pt = U_data_pt->value_pt(0);//Points the global parameter to the place where the U_data_pt is.
    U_data_pt->unpin(0);

    Q_zero_pt = &Problem_Parameter::global_Q_zero;//It points to the global variable
    *Q_zero_pt = initial_Q_zero; //Gives it an initial value

    //JACK - The next loop makes the integral measures free parameters. Pin one of them if a particular branch is required, i.e. let CoM_Y = 0 to get a branch that is symmetric - note that this can also be achieved above
    ...
    //JACK - NEED TO FIND OUT WHAT VOLUME CONSTRAINT ELEMENT DOES - IT MUST ADD VOLUME TO THE RESIDUAL
    ...
    CoM_Y_constraint_el_pt=
        new SelfReferentialVolumeConstraintElement(Problem_Parameter::global_CoM_Y_pt,
                CoM_Y_data_pt,
                index_of_traded_pressure);

   Integral_measures_el_pt = new SelfReferentialVolumeConstraintElement(Problem_Parameter::global_CoM_Y_pt,
...
// Place it smack in the middle of the channel
	...
	double minor_radius = Problem_Parameter::Minor_Radius;
        Ellipse * bubble_pt = new Ellipse(major_radius,
                                          minor_radius);
...
// Build the 1st bubble polyline
        bubble_polyline_pt[0] = new TriangleMeshPolyLine(bubble_vertex,
                First_bubble_boundary_id+2*i_circle);
...
// Build the 2nd bubble polyline
        bubble_polyline_pt[1] = new TriangleMeshPolyLine(bubble_vertex,
                Second_bubble_boundary_id+2*i_circle);
...
// Create closed polygon from two polylines
        Bubble_polygon_pt[i_circle] = new TriangleMeshPolygon(
            bubble_polyline_pt,
            bubble_center);
...
// Build global mesh
    ...
    Problem::Suppress_warning_about_actions_before_read_unstructured_meshes=true;
...
//    unsigned nb=Fluid_mesh_pt->nboundary();
    for(unsigned b=First_bubble_boundary_id; b<Second_bubble_boundary_id+1; b++)
    {
        ...
        for(unsigned e=0; e<n_element; e++)
        {
            ...
            HeleShawInterfaceElement<ELEMENT>* el_pt = new HeleShawInterfaceElement<ELEMENT> (bulk_elem_pt, face_index);
	    ...
	    //Jack - it is in the following lines that the hele_shaw_interface_file interacts with all of the parameters and functions provided in the Problem_Parameter namespace file etc
	    ...
	    /// To calculate constants for surface tension
            ...
            el_pt->bubble_pressure_fct_pt() = Problem_Parameter::bubble_pressure_function;
            ...
            unsigned measure_data_number = el_pt->add_external_data(Integral_measures_data_pt, true);
	    el_pt->integral_measure_data_number = measure_data_number;

	    el_pt->fill_solid_by_fd_pt = &Problem_Parameter::fill_in_solid_jacobian_interface_by_fd;
	    
//            el_pt->use_minisolve_state_pt = &Problem_Parameter::Minisolve_state;
        }
    }
}
    ...
    for(unsigned b=First_bubble_boundary_id; b<Second_bubble_boundary_id+1; b++)
    {
        ...
        for(unsigned e=0; e<n_element; e++)
        {
            ...
            HeleShawVolumeConstraintElement<ELEMENT>* el_pt =
                new HeleShawVolumeConstraintElement<ELEMENT>(
    ...
    for(unsigned b=First_bubble_boundary_id; b<Second_bubble_boundary_id+1; b++)
    {
        ...
        for(unsigned e=0; e<n_element; e++)
        {
            ...
            HeleShawCoMXConstraintElement<ELEMENT>* el_pt =
                new HeleShawCoMXConstraintElement<ELEMENT>(
                bulk_elem_pt,face_index);
    ...
    for(unsigned b=First_bubble_boundary_id; b<Second_bubble_boundary_id+1; b++)
    {
        ...
        for(unsigned e=0; e<n_element; e++)
        {
            ...
            HeleShawCoMYConstraintElement<ELEMENT>* el_pt =
                new HeleShawCoMYConstraintElement<ELEMENT>(
                bulk_elem_pt,face_index);
...
//                std::cout << "Pin pressure to zero on outflow boundary " << std::endl;
            }
        }
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::jack_solve(double Q_unsteady,bool jack_arc,unsigned n_iter,double sign)
{
...
template<class ELEMENT>
Vector<complex<double> > BubbleInChannelProblem<ELEMENT>::fill_in_landau_coefficients(Vector<complex<double> > &g,
										      Vector<complex<double> > &g_adjoint,
										      Vector<complex<double> > &varphi_0,
										      Vector<complex<double> > &varphi_1,
										      Vector<complex<double> > &varphi_2,
										      ...
										      double delta)
{
  ...
  //  nu = -nu;//Because we did this to get the eigenvalue calculation correct and need to be consistent
  ...
  //
  //  So we are free to interchange the arguments of the tressian - see equation 5.9 in weakly nonlinear //
  //  document
  //
  ...
  std::cout<<"============================================================="<<std::endl;
  std::cout<<"============================================================="<<std::endl;
  std::cout<<"The weakly nonlinear analysis has successfully been completed"<<std::endl;
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::fill_in_steady_fourth_order_solution(Vector<complex<double> > &varphi_3,Vector<complex<double> > &varphi_2,double delta)
{
  int N = this->ndof();
  CRDoubleMatrix jacobian(this->dof_distribution_pt());
  ...
  ofstream outdata;
  char filename[100];
  sprintf(filename,"RESLT/varphi_3.dat");
  outdata.open(filename);
  for(int i=0;i<N;i++)
    {
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::fill_in_second_order_solution(Vector<complex<double> > &g,
								    double Q_c,
								    double omega_c,
								    int mode,
								    double delta,
								    ...
								    Vector<complex<double> > &varphi_2)
{
  ...
  //Get Hessian 0.5*H[u0](g,g) = 0.5*H[u0](g_r,g_r) - 0.5*H[u0](g_i,g_i) + 0.5*i*H[u0](g_r,g_i) + 0.5*i*H[u0](g_i,g_r);
  ...
  sprintf(filename,"RESLT/varphi_0.dat");
  ...
  //        We need
  //
  //        i*omega_c*(-M[g*]g + M[g]g*) = i*omega_c*2.0*i*(-M[g_r]g_i + M[g_i]g_r)
  //
  //
  ////////////////////////////////////////////////////////
  ...
  //Get Hessian 0.5*H[u0](g,g*) + 0.5*H[u0](g*,g)= H[u0](g_r,g_r) + H[u0](g_i,g_i)
  ...
  sprintf(filename,"RESLT/varphi_1.dat");
  ...
  sprintf(filename,"RESLT/varphi_2.dat");
						   ...
						   Vector<complex<double> > &f)
{
  ...
  if(order == 2)
    {
      std::cout<<" "<<std::endl;
      std::cout<<"Now calculating the real part of the second order Tressian"<<std::endl; 
  ...
  if(order == 4)
    {
      std::cout<<" "<<std::endl;
      std::cout<<"Now calculating the real part of the fourth order Tressian"<<std::endl; 
  ...
  if(order == 2)
    {
      std::cout<<" "<<std::endl;
      std::cout<<"Now calculating the imag part of the second order Tressian"<<std::endl; 
  ...
  if(order == 4)
    {
      std::cout<<" "<<std::endl;
      std::cout<<"Now calculating the imag part of the fourth order Tressian"<<std::endl; 
						    ...
						    complex<double> mult)
{
  ...
  sprintf(filename,"RESLT/grifter1.dat");
  outdata.open(filename);
						    ...
						    double mult)
{
  ...
  identity.sparse_indexed_output_with_offset("RESLT/sparse_identity.dat");
							   ...
							   DoubleVector &f)
{
  ...
  ofstream outfile;
  char filename[100];
  sprintf(filename,"RESLT/test_hessian.dat");
...
void BubbleInChannelProblem<ELEMENT>::test_identity_matrix()
{
  ...
  std::cout<<"Number of non-zeros entries of J*I "<<result1.nnz()<<std::endl;    
  std::cout<<"Number of non-zeros entries of J+J "<<result2.nnz()<<std::endl;
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::create_complex_mass_matrix(CRComplexMatrix &mass_complex,complex<double> mult)
{
  ...
  mass.sparse_indexed_output_with_offset("RESLT/sparse_mass.dat");

  //Output the CRDouble jacobian
  jacobian.sparse_indexed_output_with_offset("RESLT/sparse_jacobian.dat");
  ...
  //Build the CRComplex column and row vectors directly from the known distribution of the CRDoubleMatrix
  Vector<int> col(nnz);
  Vector<int> row(N+1);
  ...
  ofstream outdata;
  char filename[100];
  sprintf(filename,"RESLT/sparse_complex_mass.dat");
  outdata.open(filename);
  for(long int i=0;i<N;i++)
    {
      for(int j=row[i];j<row[i+1];j++)
	{
	  outdata<<i<<" "<<col[j]<<" "<<real(value[i])<<" "<<imag(value[j])<<std::endl;
	}
    }
  outdata.close();
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::create_complex_jacobian_matrix(CRComplexMatrix &jacobian_complex,complex<double> mult)
{
  //Create the space for the mass and jacobian matrices
  CRDoubleMatrix mass(this->dof_distribution_pt());
  CRDoubleMatrix jacobian(this->dof_distribution_pt());
  ...
  ofstream outdata;
  char filename[100];
  sprintf(filename,"RESLT/sparse_complex_jacobian.dat");
  outdata.open(filename);
  for(long int i=0;i<N;i++)
    {
      for(int j=row[i];j<row[i+1];j++)
...
void BubbleInChannelProblem<ELEMENT>::test_complex_identity_matrix()
{
  ...
  //Build the CRComplex column and row vectors directly from the known distribution of the CRDoubleMatrix mass
  Vector<int> col(nnz);
  Vector<int> row(N+1);
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::test_mass_matrix(Vector<complex<double> > &g)
{
  int N = this->ndof();
  CRDoubleMatrix mass(this->dof_distribution_pt());
  CRDoubleMatrix jacobian(this->dof_distribution_pt());
  ...
  ofstream outdata;
  char filename[100];
  sprintf(filename,"RESLT/mass_g.dat");
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::solve_landau_equation(double tend,
							    ...
							    double epsilon)
{
  ...
  std::cout<<" "<<std::endl;
  std::cout<<std::setw(20)<<"Time"<<std::setw(20)<<"A[0]"<<std::setw(20)<<"A[1]"<<std::endl;
  std::cout<<std::setw(20)<<t<<std::setw(20)<<u[0]<<std::setw(20)<<u[1]<<std::endl;
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::solve_landau_equation(Vector<double> &solution,
							    double dt,
							    double t,
							    complex<double> &lambda,
							    complex<double> &mu,
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::get_landau_residual(Vector<double> &u,
							  Vector<double> &f,
							  complex<double> &lambda,
							  complex<double> &mu,
							  double epsilon)
{
  f[0] = epsilon*epsilon*(real(lambda)*u[0] + real(mu)*u[0]*u[0]*u[0]);
  f[1] = epsilon*epsilon*(imag(lambda) + imag(mu)*u[0]*u[0]);
}

template<class ELEMENT>
int BubbleInChannelProblem<ELEMENT>::create_bifurcation_diagram(double delta,
								complex<double> lambda,
								complex<double> mu,
								bool hopf1)

 {
   ...
   std::cout<<" "<<std::endl;
	...
	if(hopf1 == true)
	  {
	    std::cout<<std::setw(100)<<"----------------------------H1============================"<<std::endl;
	  }
	else
	  {
	    std::cout<<std::setw(100)<<"----------------------------H2============================"<<std::endl;
	  }  
	std::cout<<std::setw(100)<<"                           =                              "<<std::endl;
	std::cout<<std::setw(100)<<"                          =                               "<<std::endl;
	std::cout<<std::setw(100)<<"                         =                                "<<std::endl;
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::get_inner_product(Vector<complex<double> > &f,Vector<complex<double> > &g,complex<double> &ans)
{
  int N = this->ndof();
  for(int i=0;i<N;i++)
    {
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::test_tressian_convergence_new(Vector<complex<double> > &g,
								    Vector<complex<double> > &g_adjoint)
{
  int N = this->ndof();
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::get_tressian_new(Vector<complex<double> > &g1,Vector<complex<double> > &g2,Vector<complex<double> > &g3,vector<double> &u0,Vector<complex<double> > &f)
{
  int N = this->ndof();
  CRDoubleMatrix mass(this->dof_distribution_pt());
  CRDoubleMatrix jacobian_plus_1(this->dof_distribution_pt());
  ...
  //
  //     H[u0][u0+eps*g1]*g2 = (1/2*eps)*( J[u0 + eps*g1 + eps*g2] - J[u0 + eps*g1 - eps*g2])
  //
  //     H[u0][u0-eps*g1]*g2 = (1/2*eps)*( J[u0 - eps*g1 + eps*g2] - J[u0 - eps*g1 - eps*g2])
  //
  //
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::get_tressian_new1(DoubleVector &g1,DoubleVector &g2,DoubleVector &g3,Vector<double> &u0,DoubleVector &f)
{
  int N = this->ndof();
  CRDoubleMatrix mass(this->dof_distribution_pt());
  CRDoubleMatrix jacobian_plus_1(this->dof_distribution_pt());
  ...
  //     T[u0](g1,g2) = (1/2*eps)*( H[u0+eps*g1]g2 - H[u0-eps*g1]g2 )
  //
  //     H[u0][u0+eps*g1]*g2 = (1/2*eps)*( J[u0 + eps*g1 + eps*g2] - J[u0 + eps*g1])
  //
  //     H[u0][u0-eps*g1]*g2 = (1/2*eps)*( J[u0 - eps*g1 + eps*g2] - J[u0 - eps*g1])
  //
  //
...
template<class ELEMENT>
double BubbleInChannelProblem<ELEMENT>::test_taylor_series(Vector<complex<double> > &g,double eps)
{
  int N = this->ndof();
  Vector<double> u0(N);
  Vector<double> gg(N);
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::doc_weakly_nonlinear_solution(Vector<double> &u,
								    double t,
								    complex<double> &lambda,
								    complex<double> &mu,
								    double eps,
								    ...
								    Vector<complex<double> > &varphi_2)
{
  ...
  for(int i=0;i<N;i++)
    {
      ...
      this->dof(i) += 2.0*eps*r*(real(g[i])*cos(Delta) - imag(g[i])*sin(Delta));

      this->dof(i) += 2.0*eps*eps*r*r*(real(varphi_0[i])*cos(2.0*Delta) - imag(varphi_0[i])*sin(2.0*Delta));
  ...
  sprintf(filename,"%s/boundaries_weakly_nonlinear_eps_%i_theta_%i_doc_%i.dat",
	  Problem_Parameter::Doc_info.directory().c_str(),
  ...
  if(Problem_Parameter::Doc_info.number() == 0)
    {
      std::cout<<" "<<std::endl;
      std::cout<<std::setw(20)<<"Time"<<std::setw(20)<<"A[0]"<<std::setw(20)<<"A[1]"<<std::setw(20)<<"eps"<<std::setw(20)<<"Ub"<<std::setw(20)<<"pb"<<std::setw(20)<<"omega_c"<<std::endl;
      std::cout<<std::setw(20)<<t<<std::setw(20)<<u[0]<<std::setw(20)<<u[1]<<std::setw(20)<<eps<<std::setw(20)<<get_U()<<std::setw(20)<<get_P()<<std::setw(20)<<omega_c<<std::endl;
    }
  else
    {
      std::cout<<std::setw(20)<<t<<std::setw(20)<<u[0]<<std::setw(20)<<u[1]<<std::setw(20)<<eps<<std::setw(20)<<get_U()<<std::setw(20)<<get_P()<<std::setw(20)<<omega_c<<std::endl;
  ...
  Problem_Parameter::Trace_weakly_nonlinear<<t<<" "
					   <<u[0]<<" "
					   <<u[1]<<" "
					   <<eps<<" "
					   <<get_U()<<" "
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::set_initial_condition_weakly(Vector<double> &u,
								   double t,
								   complex<double> &lambda,
								   complex<double> &mu,
								   double eps,
...
{
  ifstream* restart_file_pt = new ifstream(CommandLineArgs::Argv[1],ios_base::in);
