                              ...
                              CRComplexMatrix& result) const
{
 ...
 if ((method == 1) || (method == 2) || (method == 3))//JACK
  {
   ...
   // --------
   else if (method==2)
    {
     ...
     std::map<int,std::complex<double> >* result_maps = new std::map<int,std::complex<double> >[N];
	...
	// run through non-zeros in this_row
	  ...
	  // find value of non-zero
         ...
         for (int matrix_in_ptr = matrix_in_row_start[matrix_in_row];
          ...
          {
           // find column index for non-zero in matrix_in
	    int col = matrix_in_column_index[matrix_in_ptr];//This is the offending line
           // insert value
     ...
     for (unsigned long row=0; row<N; row++)
      {
       ...
       for (std::map<int,std::complex<double> >::iterator i = result_maps[row].begin();
                         ...
                         CRComplexMatrix &result_matrix) const
{
 ...
 const std::complex<double>* this_values = this->value();//JACK
 const std::complex<double>* in_values = matrix_in.value();//JACK
 ...
 for (unsigned row_i = 0; row_i < nrow_local; row_i++) 
  {
   ...
   for(std::map<int,std::complex<double> >::iterator it = res_row_map.begin();it != res_row_map.end(); ++it)
    {
 ...
 result_matrix.build(res_values,res_column_indices,res_row_start,this->ncol(),this->nrow());
                         ...
                         CRComplexMatrix &result_matrix) const
{
 ...
 // Values from this matrix and matrix_in.
 const std::complex<double>* this_values = this->value();//JACK
 const double* in_values = matrix_in.value();//JACK
