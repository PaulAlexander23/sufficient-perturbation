  ...
  Vector<Vector<double> > polygon_vertices,Vector<double> &point_inside_coord)
{
    ...
    {
      if (polygon_vertices[i][0]>max_x){max_x = polygon_vertices[i][0];}
      if (polygon_vertices[i][0]<min_x){min_x = polygon_vertices[i][0];}	
      if (polygon_vertices[i][1]>max_y){max_y = polygon_vertices[i][1];}
      if (polygon_vertices[i][1]<min_y){min_y = polygon_vertices[i][1];}
    }
        ...
        for (unsigned i_bisect = 0; i_bisect<max_bisections_to_search; i_bisect++)
        {
            ...
            unsigned intersections = UnstructuredTwoDMeshGeometryBase::is_point_inside_polygon_intersections(polygon_vertices, test_coord);
        ...
        bool is_point_inside = UnstructuredTwoDMeshGeometryBase::is_point_inside_polygon_helper(polygon_vertices, point_inside_coord);
        if (is_point_inside==false)
        {
            std::cout << "Failed to find a point inside the polygon in a " << max_bisections_to_search << " step bisection search" << std::endl;
 ...
 {
    unsigned intersect_counter = is_point_inside_polygon_intersections(polygon_vertices,point);
 ...
 //======================================================================
 /// Helper function that checks how many intersections there are between a given point,
 /// a point to its far right, and a polygon.
 /// If there are an odd number of intersections, the point is inside this polygon.
 /// Knowing the number of intersections (rather than just true or false) allows us to 
 /// use a bisection search to identify a point inside the polygon, even if it is convex.
 //======================================================================
 unsigned UnstructuredTwoDMeshGeometryBase::is_point_inside_polygon_intersections(
  Vector<Vector<double> > polygon_vertices,Vector<double> point)
 {
