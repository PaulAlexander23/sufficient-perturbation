...
/// Update the problem specs before solve
    ...
    void get_tressian(DoubleVector &g1,DoubleVector &g2,vector<double> &u0,DoubleVector &f);
    
    void get_mass(DoubleVector &g1,DoubleVector &g2,vector<double> &u0,DoubleVector &f);
      ...
      for(int i=1;i<N_eps;i++)
	{
      ...
      ofstream outdata;
      char filename[100];
      sprintf(filename,"RESLT/eigenmode_data_com_y.dat");
	  ...
	  for(int m=0;m<N_theta;m++)
	    {
	      for(unsigned i=0;i<n_dof;i++)
		{
		  dof(i) = backup_1[i] + eps[l]*(cos(theta[m])*eigenvectors[eigenmode][i] - sin(theta[m])*eigenvectors[conjugate][i]);
		}
	      ...
	      //      actions_after_change_in_bifurcation_parameter();
	      outdata <<eps[l]<<" "<<theta[m]<<" "<<get_CoM_Y()<<" "<<get_y_max()<<" "<<get_y_min()<<std::endl;
		}
	}
      outdata.close();
      exit(1);
	...
	{
	  epsilon = 0.0;
	  sprintf(filename,"RESLT/eigenmode_data_eps_theta_%i.dat",j);
	  outdata.open(filename);
	  for(int m=0;m<30;m++)
	    {
	      for(unsigned i=0;i<n_dof;i++)
		{
		  dof(i) = backup_1[i] + epsilon*(cos(theta[j])*eigenvectors[eigenmode][i] - sin(theta[j])*eigenvectors[conjugate][i]);
		}
	      actions_after_change_in_bifurcation_parameter();
	      outdata << theta[j]<<" "<<get_CoM_Y()<<" "<<get_y_max()<<" "<<get_y_min()<<" "<<epsilon<<std::endl;
	      epsilon += 1.0;
	    }
	  outdata.close();
	}
      ...
      for(int i=0;i<N;i++)
	{
      ...
      complex<double> evalue1 = (inn_prod2_real + 1i*inn_prod2_imag)/(inn_prod1_real + 1i*inn_prod1_imag);
      ...
      for(unsigned m=0;m<1000;m++)
	{
	  if(answer==0)
	    {
	      ...
	      std::cout<<"====================="<<std::endl;
	      std::cout<<"====================="<<std::endl;
	      std::cout<<"What is your first eigenmode? (0-9)"<<std::endl;      	      std::cout<<"====================="<<std::endl;
	      std::cout<<"====================="<<std::endl;
	      std::cin >> eigenmode1;
	      std::cout<<"What is the conjugate? (0-9)"<<std::endl;	  
	      std::cout<<"====================="<<std::endl;
	      ...
	      std::cout<<"====================="<<std::endl;
	      std::cout<<"====================="<<std::endl;
  	      std::cout<<"What is your second eigenmode? (0-9)"<<std::endl;      	      std::cout<<"====================="<<std::endl;
	      std::cout<<"====================="<<std::endl;
	      std::cin >> eigenmode2;
	      std::cout<<"What is the conjugate? (0-9)"<<std::endl;	  
	      std::cout<<"====================="<<std::endl;
	      ...
	      std::cout<<"====================="<<std::endl;
	      std::cout<<"====================="<<std::endl;
	      std::cout<<"Sure you're happy? (true = 1/false = 0)"<<std::endl;	  	      std::cout<<"====================="<<std::endl;
	      std::cout<<"====================="<<std::endl;
	      std::cin >> answer;
	    }
	  else
      ...
      some_file <<eigenmode1<<" "<< epsilon1<<" "<< " "<<theta1<<endl;
      some_file <<eigenmode2<<" "<< epsilon2<<" "<< " "<<theta2<<endl;
      some_file.close();

      for(unsigned i =0;i<n_dof;i++)
	{
	  dof(i) = 100*(cos(theta1)*eigenvectors[eigenmode1][i] - sin(theta1)*eigenvectors[conjugate1][i]);//First eigenmode
	  dof(i) += 100*(cos(theta1)*eigenvectors[eigenmode2][i] - sin(theta2)*eigenvectors[conjugate2][i]);//First eigenmode
	}

      for(unsigned i=0;i<n_dof;i++)
	{
	  dof(i) = backup_1[i] + epsilon1*(cos(theta1)*eigenvectors[eigenmode1][i] - sin(theta1)*eigenvectors[conjugate1][i]);
	  dof(i) += epsilon2*(cos(theta2)*eigenvectors[eigenmode2][i] - sin(theta2)*eigenvectors[conjugate2][i]);
	}
      doc_solution();
      ...
      for(unsigned m=0;m<n_eval;m++)
	{
      ...
      sprintf(filename,"RESLT/perturbation.dat");
      
      some_file.open(filename);
      some_file <<eigenmode<<" "<< eps<<" "<< " "<<theta<<endl;
      some_file.close();

      for(unsigned i=0;i<n_dof;i++)
	{
	  dof(i) = backup_1[i] + eps*(cos(theta)*eigenvectors[0][i] - sin(theta)*eigenvectors[1][i]);
	}
    ...
    void output_eigenvector_perturbation(int eps,int theta)
    {
      ...
      ofstream some_file;
      char filename[100];
      sprintf(filename,"%s/perturbation_eps_%i_theta_%i.dat",
...
/// Pointer to Fluid_mesh
    ...
    //Jack - this is where the Bubble_polygon_pt is defined and it is public to the problem
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::fill_in_landau_coefficients(Vector<complex<double> > &g,
								  Vector<complex<double> > &g_adjoint,
								  Vector<complex<double> > &varphi_0,
								  Vector<complex<double> > &varphi_2,
								  Vector<complex<double> > &varphi_3,
...
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::fill_in_steady_fourth_order_solution(Vector<complex<double> > &varphi_4,Vector<complex<double> > &varphi_3,int delta)
{
  int N = this->ndof();
  CRDoubleMatrix jacobian(this->dof_distribution_pt());
  ...
  ofstream outdata;
  char filename[100];
  sprintf(filename,"RESLT/varphi_4.dat");
  outdata.open(filename);
  for(int i=0;i<N;i++)
    {
								    ...
								    Vector<complex<double> > &varphi_3)
{
  ...
  //Solve the equation (real and imag parts separately as jacobian is purely real)
...
{
 ifstream* restart_file_pt = new ifstream(CommandLineArgs::Argv[2],ios_base::in);
 restart(*restart_file_pt);
}
