...
//                residuals[measure_eqn] += W * J * kappa*kappa;
            ...
            for (unsigned i_measure = 0; i_measure<integral_values; i_measure++)
            {
                ...
                double pressure_inside_bubble = p_bubble + 1.0/(3.0*alpha*Q*b);
...
//    }
    ...
    if (flag==1)
    {
        if (use_fd_for_dresidual_dnodal_coordinates_pt == 0 || *use_fd_for_dresidual_dnodal_coordinates_pt == true)
        {
            this->fill_in_jacobian_from_solid_position_by_fd(jacobian);
        }
        else
            ...
            for (unsigned n=0; n<3; n++)
            {
                for (unsigned d=0; d<2; d++)
                {
                    ...
                    if (local_eqn>=0)
                    {
                        ...
                        {
//                            if (std::abs(jacobian(u,local_eqn)-d_residual_dnodal(u,d,n))>1e-6)
//                                {
//                                    std::cout << n  << " " << d << " " << u << " " << jacobian(u,local_eqn) << " " << d_residual_dnodal(u,d,n) << std::endl;
//                                }
//                            else
//                            {
...
//    void output_interface_info(const double&s,  double& U_dot_n, double& Rt_dot_n,  Vector<double>& interpolated_x,Vector<double>& interpolated_t1)
//    {
//
...
//                interpolated_t1[0] += this->nodal_position (l, 0) * dpsifds (l, 0);
//                interpolated_t1[1] += this->nodal_position (l, 1) * dpsifds (l, 0);
//                interpolated_dx_dt[0] += this->dnodal_position_dt(l,0)*psif(l);
//                interpolated_dx_dt[1] += this->dnodal_position_dt(l,1)*psif(l);
//            }
//                                //Now calculate the unit normal vector
//        Vector<double> interpolated_n(2);
...
//
//
//        double curvature =(interpolated_Xdd*interpolated_t1[1] - interpolated_Ydd*interpolated_t1[0])/(J*J);
//        curvature = kappa;
//
...
//                                W * psif(l)*(interpolated_p-bubble_pressure + Ca_inv_k * curvature)*dJ_dX(p,q);
//
...
//
//                              dresidual_dnodal_coordinates(local_eqn,p,q) +=
//                                W * psif(l) * J * Ca_inv_k *(interpolated_Xdd*dn0_dX(p,q) + interpolated_Ydd*dn1_dX(p,q));
//
//                            /// Terms due to curvature - not needed if curvature is in bubble pressure.
////                            dresidual_dnodal_coordinates(local_eqn,p,q) +=
////                                -W*psif(l)*Ca_inv_k *(interpolated_Xdd*d_t1_1_dX(p,q) - interpolated_Ydd*d_t1_0_dX(p,q))/J;
//
//
//                        }
//
...
//                        {
//                            dresidual_dnodal_coordinates(local_eqn,p,q) +=
//                            -interpolated_lagrange * interpolated_n[0] *psif(l) *W* dJ_dX(p,q);
//
//                            dresidual_dnodal_coordinates(local_eqn,p,q) +=
...
//                        {
//                            dresidual_dnodal_coordinates(local_eqn,p,q) +=
//                            -interpolated_lagrange * interpolated_n[1] *psif(l) *W* dJ_dX(p,q);
//
//                            dresidual_dnodal_coordinates(local_eqn,p,q) +=
...
//    {
//
////        std::cout << "Calling interface mass matrix with multiplier " << *construct_mass_matrix_left_multiplier_pt << std::endl;
...
//        /// For node = 1, this is the only element that will contribute to the mass matrix
//
//        if (construct_mass_matrix_left_multiplier_pt==0 || *construct_mass_matrix_left_multiplier_pt == false)
//        {
...
//        }
//
//        /// Alice: For my sanity later on. External dependencies must still be added in driver code.
//        /// May be useful for finding steady solutions where frame speed or capillary number
//        /// are free parameters. The residuals depend linearly on these parameters, so fd
