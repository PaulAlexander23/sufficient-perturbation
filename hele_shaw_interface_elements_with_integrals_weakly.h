...
public:
    ...
    /// \short Function pointer to function which provides bubble pressure as a function
    /// of vector x. This should usually be a constant - should default to zero.
    /// Any perturbations to the system can be supplied through a spatially varying
    /// bubble pressure function.
    typedef void (*BubblePressureFctPt)(const Vector<double>& x, double& p_bubble);

    typedef void (*BubblePressureGradientFctPt)(const Vector<double>& x, Vector<double>& grad_pb);
    ...
    /// \short Function pointer to function which provides wall speed as a function of x.
    /// This allows us to solve for bubble motion in a moving frame. A constant wall speed
    /// does not affect the mass conservation equations, but does feature in the kinematic equation
    /// for interface motion.
    typedef void (*WallSpeedFctPt)(const Vector<double>& x, Vector<double>& U_wall);


    BubblePressureFctPt& bubble_pressure_fct_pt() {return Bubble_pressure_fct_pt;}
    BubblePressureFctPt bubble_pressure_fct_pt() const {return Bubble_pressure_fct_pt;}
    BubblePressureGradientFctPt& bubble_pressure_gradient_fct_pt() {return Bubble_pressure_gradient_fct_pt;}
...
/// Return the Lagrange multiplier at local node j
    ...
    int tangential_lagrange_local_eqn(const unsigned &j)
    {
        // Get the index of the nodal value associated with Lagrange multiplier
        // NOTE: It's the first new one
        unsigned tangential_lagr_index=dynamic_cast<BoundaryNodeBase*>(node_pt(j))->
                            index_of_first_value_assigned_by_face_element(Id)+3;
...
/// this element (in order: 0: Lagrange multiplier; 1: x-component of
/// smoothed derivative of tangent vector; 2: y-component of
...
/// Access function: Pointer to source function. Const version
    ...
    const double& q_inv() const {return *Q_inv_pt;}

    /// The value of the Q_zero
    const int& q_zero() const {return *Q_zero_pt;}
    
    /// Pointer to the inverse Capillary number
    double*& q_inv_pt() {return Q_inv_pt;}

    /// Pointer to Q_zero_pt
    int*& q_zero_pt() {return Q_zero_pt;}
    ...
    double*& measure_pt() {return Measure_pt;}
...
//    std::cout << "Measure is data " << integral_measure_data_number << " with " << external_data_pt(integral_measure_data_number)->nvalue() << " values " << std::endl;
...
//            interpolated_Ys += node_pt(l)->value(4)*psif[l];
        ...
        //Get the value of the bubble pressure. This need not be constant, and in fact currently
        // contains the transverse curvature term - JACK - this is where ther transverse curvature term is contained, i.e.
...
//     sigma_kappa+=sigma_local*Ca_inv*tau[k]*interpolated_n[k];
        ...
        double direction = interpolated_n[0]*interpolated_tangent[1] - interpolated_n[1]*interpolated_tangent[0];
        double sign = 0;
                ...
                if(local_eqn >= 0)
                {
                    ...
                    if(flag==1)
                    {
                        ...
                        for(unsigned l2=0; l2<n_node; l2++)
                        {
                            ...
                            if(local_unknown >= 0)
                            {
//                            std::cout << "Interface 601" << local_eqn << " " << local_unknown << std::endl;
		...
		if(Q_zero==1)
		  {
		    ...
		    residuals[local_eqn] +=
		      (interpolated_p-p_bubble + kappa/(3.0*alpha*alpha*Q))*psif(l)*W*J;

		    //Full equations - analysis in Q
		    //    residuals[local_eqn] +=
		    //(Q*(interpolated_p - p_bubble) + kappa/(3.0*alpha*alpha))*psif(l)*W*J;
		...
		if(Q_zero==0)
		  {
		    //Set Q_inv = 0
		    residuals[local_eqn] += (interpolated_p - p_bubble)*psif(l)*W*J;
		    ...
		    //residuals[local_eqn] += (kappa/(3.0*alpha*alpha))*psif(l)*W*J;
			  ...
			  if(Q_zero == 0)
			      ...
			      if(Q_zero == 1)
				{
				  //Full equations if analysis in Q_inv
				  jacobian(local_eqn, local_unknown) +=
                                    psif(l)*W*J*psif(l2)*interpolated_n[i2]/(3.0*alpha*alpha*Q);	      
...
//                std::cout << "Tangential lagrange is not pinned" << std::endl;
...
//                residuals[measure_eqn] += W * J * kappa*kappa;
            ...
            unsigned integral_values = external_data_pt(integral_measure_data_number)->nvalue();
	    ...
	    for (unsigned i_measure = 0; i_measure<integral_values; i_measure++)
		{
                ...
                double pressure_inside_bubble = 0.5;//Q_zero*p_bubble + 1.0/(3.0*alpha*b*Q);//Jack

		if((Q_zero != 1) && (Q_zero != 0))
		  {
		    std::cout<<" "<<std::endl;
		    std::cout<<"Oh oh, this ain't gonna work - need to set_Q_zero to either 0 or 1 and you haven't"<<std::endl;
		    exit(1);
		  }
		...
		//                std::cout << pressure_inside_bubble << std::endl;
                ...
                local_eqn = external_local_eqn(integral_measure_data_number, i_measure);
                if (local_eqn>=0)
                {
                    if (i_measure==0)
                    {
                    ...
                    {
                        /// The projected area of the bubble can be written as an integral over the interface
                        /// by using the divergence theorem (or equivalently Green's theorem)
		      ...
		      ///residuals[local_eqn] += y*n_y*ds;
                    ...
                    {
                        /// We can construct the total surface area: 2 projected areas, and 1 vertical sides
		      ...
		      residuals[local_eqn] += (pressure_inside_fluid-pressure_inside_bubble+1.0/(3.0*alpha*Q)*(1.0/b+kappa/alpha))*ds;

		      //If analysis in Q
		      //residuals[local_eqn] += (pressure_inside_fluid-pressure_inside_bubble+1.0/(3.0*alpha)*(1.0/b+kappa/alpha))*ds;
		      
                    }
		    else if (i_measure==12)
		    { 
		      ///JACK - THIS SHOULD INTEGRATE TO THE AREA OF THE BUBBLE ALSO - Actually using Green's Theorem Area = (1/2)*integral_{bounding curve} xdx + ydy. Therefore Area = integral_{bounding curve} xdx = integral_{bounding_curve}ydy. Not int_{curve} y dy = int_{curve} y*dy/ds ds = int_{curve} y*n_y ds as required.
		      residuals[local_eqn] += y*n_y*ds;
		    }
                }
            }
...
//                local_eqn = this->position_local_eqn(l,i);
                ...
                if(local_eqn >= 0)
                {
                    ...
                    residuals[local_eqn] +=
                        interpolated_tangential_lagrange * interpolated_tangent[i] * psif[l] *W*J*sign;
            ...
            if (flag==3)
            {
                for (unsigned l2 =0; l2<n_node; l2++)
                {
                    ...
                    if (local_unknown>=0 && local_eqn>=0)
                    {
                        ...
                        += h*St*psif(l)*W*J*interpolated_n[0]*psif(l2);
//                        std::cout << "Adding " << h*St*psif(l)*W*J*interpolated_n[0]*psif(l2) << "to " << local_unknown << " " << local_eqn << std::endl;
////                        mass_matrix(local_unknown,local_eqn)
////                        += 1;
                    }
...
//                        mass_matrix(local_unknown,local_eqn)+= 1;
                    ...
                    {
//                        std::cout << "Adding 1 to " << local_eqn << " " << local_unknown << std::endl;
                        mass_matrix(local_unknown, local_eqn)=1;
...
template<class ELEMENT>
void HeleShawInterfaceElement<ELEMENT>::
get_dresidual_dnodal_coordinates(RankThreeTensor<double>& dresidual_dnodal_coordinates)
    {
	  ...
	  for (unsigned l = 0; l < n_node; l++) // Looping over shape function.
	      ...
	      interpolated_t1[0] += this->nodal_position (l, 0) * dpsifds (l, 0); //Tangent vector
	      interpolated_t1[1] += this->nodal_position (l, 1) * dpsifds (l, 0); //Tangent vector
	  ...
	  //Jack - no longer seg faults but need to fill in the actual details in the custom_hele_shaw etc. file
	  ...
	  for(unsigned l=0; l<n_node; l++)
		  ...
		  for(unsigned q=0; q<n_node; q++)
		      ...
		      // First scope is basically differentiating (2) w.r.t X - the nodal coordinates(which come as part of the solution)
		      //


		      //  residuals[local_eqn] +=(interpolated_p-p_bubble + kappa/(3.0*alpha*alpha*Q))*psif(l)*W*J;               (2)
		      
		      local_eqn = this->nodal_local_eqn(l,1);
		      
		      if (local_eqn>=0)
                        {
			  // Firstly, terms due to Jacobian varying (but no curvature changes) TICK
			  dresidual_dnodal_coordinates(local_eqn,p,q) +=
			    W * psif(l)*(interpolated_p-bubble_pressure + kappa/(3.0*alpha*alpha*Q))*dJ_dX(p,q);//This comes from differentiating J w.r.t X in (2)
			  
			  // Terms due to pb varying- JACK - why no extra term?
			  //This comes from differentiating p_bubble w.r.t X - grad_pb = dp/dX = dp/dy*dy/dX + dp/dx*dx/dX;
			  ...
			  dresidual_dnodal_coordinates(local_eqn,p,q) +=
			    W * psif(l) * J * 1.0/(3.0*alpha*alpha*Q) *(interpolated_Xdd*dn0_dX(p,q) + interpolated_Ydd*dn1_dX(p,q)); //What is this? What is interpolated Xdd?
			  // Terms due to curvature - not needed if curvature is in bubble pressure.
			  //This looks like -W*psi*Ca_inv_k*(Xdd \dot dt/dX)

			  //This is the first term from above
			  //dresidual_dnodal_coordinates(local_eqn,p,q) +=
			  //-W*psif(l)*1.0/(3.0*alpha*alpha*Q) *(interpolated_Xdd*d_t1_1_dX(p,q) - interpolated_Ydd*d_t1_0_dX(p,q))/J; //What is this? - probably the curavture term
		      ...
		      //  residuals[local_eqn] += h*sum(U_wall[k]*interpolated_n[k])*psif(l)*W*J;            (4)
			  ...
			  //looks like U_b*psif*W*J*b*d/dX(n) - tick don't need the U_wall[1] component because it is zero
			  dresidual_dnodal_coordinates(local_eqn,p,q) +=
		      ...
		      //  residuals[local_eqn] +=(tau[i]*psif(l)*J+interpolated_tangent[i]*dpsifds(l,0))*W;                       (1)
		      ...
		      //TICK
		      //  residuals[local_eqn] +=-interpolated_lagrange*interpolated_n[i]*psif[l]*W*J;                            (5)
		      ...
		      if (local_eqn>=0)
                        {
			  dresidual_dnodal_coordinates(local_eqn,p,q) +=
                            -interpolated_lagrange * interpolated_n[0] *psif(l) *W* dJ_dX(p,q); //Differentiating the J term in (5)
			  
			  dresidual_dnodal_coordinates(local_eqn,p,q) +=         //Differentiating the interpolated n in (5)
		      ...
		      if (local_eqn>=0)
                        {
			  dresidual_dnodal_coordinates(local_eqn,p,q) +=
                            -interpolated_lagrange * interpolated_n[1] *psif(l) *W* dJ_dX(p,q); //As above
