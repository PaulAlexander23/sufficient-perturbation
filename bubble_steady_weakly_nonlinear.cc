...
// Write trace file
    ...
    for (unsigned i_measure =0; i_measure<Problem_Parameter::n_integral_measures; i_measure++)
    {
        std::cout << i_measure << " " << Integral_measures_data_pt->value(i_measure) << std::endl;
...
// Increment the doc_info number
    Problem_Parameter::Doc_info.number()++;
 //Restart file
    sprintf(filename,"%s/restart_%i.dat",
            Problem_Parameter::Doc_info.directory().c_str(),
            Problem_Parameter::Doc_info.number());
    some_file.open(filename);
    dump_it(some_file);
...
//========================================================================
template<class ELEMENT>
void BubbleInChannelProblem<ELEMENT>::doc_solution_trace(const std::string& comment)
{
    oomph_info << "Docing step: " << Problem_Parameter::Doc_info.number()
    << std::endl;
...
/// The system is currently set up with 12 integral measures (search for i_measure in hele_shaw_interface_elements_with_integrals.h).
    /// But that number can be changed here
    ...
    problem.solve_for_adjoint_eigenproblem(adjoint_eigenvalues,adjoint_eigenvectors);

    if(Problem_Parameter::hopf1==true)
      {
	for(int i = 0;i<N;i++)
	  {
	    g_adjoint[i] = 1.0*adjoint_eigenvectors[0][i] + 1i*adjoint_eigenvectors[1][i];
	...
	for(int i = 0;i<N;i++)
	  {
	    g_adjoint[i] = 1.0*adjoint_eigenvectors[1][i] + 1i*adjoint_eigenvectors[2][i];
    ...
    problem.output_adjoint_eigenvectors_and_eigenvalues(adjoint_eigenvalues,adjoint_eigenvectors);
    ...
    problem.fill_in_second_order_solution(g,Q_c,omega_c,mode,delta,varphi_0,varphi_1,varphi_2);
    
   //Get the Landau coefficients
    coeffs = problem.fill_in_landau_coefficients(g,g_adjoint,varphi_0,varphi_1,varphi_2,Q_c,omega_c,delta);
