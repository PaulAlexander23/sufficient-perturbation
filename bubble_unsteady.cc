...
// Open trace file
    ...
    Problem_Parameter::Major_Radius=maj_rad;//*0.46//Ellipse1 = 0.5,Ellipse2=0.6,Ellipse3=0.9
    ...
    std::cout<<"Volume is "<<MathematicalConstants::Pi*Problem_Parameter::Major_Radius*Problem_Parameter::Minor_Radius<<std::endl;
...
/// The system is currently set up with 12 integral measures (search for i_measure in hele_shaw_interface_elements_with_integrals.h).
    ...
    some_file.open(filename);
    some_file << Problem_Parameter::Major_Radius  << " " << Problem_Parameter::Minor_Radius  << " " << Problem_Parameter::ycenter<<" "<< problem.get_Q() << "\"\n";
    some_file.close();
...
///   --------------- unsteady solver -----------------
      ...
      std::cout << "No. of points on the circle " << problem.get_circpts() << std::endl;
